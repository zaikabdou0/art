...
const NovaUltra = { command: ['افتح'], description: 'فتح المجموعة', elite: 'off', group: true, prv: false, lock: 'off' };
...
async function execute({ sock, msg, args }) {
    ...
    try {
        const meta = await sock.groupMetadata(chatId);
        if (!meta.announce) return reply(sock, chatId, '《✧》 المجموعة مفتوحة بالفعل.', msg);
        ...
        if (timeout > 0) {
            await reply(sock, chatId, `❀ سيتم فتح المجموعة بعد ${clockStr(timeout)}.`, msg);
            setTimeout(async () => {
                try { const md = await sock.groupMetadata(chatId); if (md.announce) { await sock.groupSettingUpdate(chatId, 'not_announcement'); await reply(sock, chatId, '✿ تم فتح المجموعة.', msg); } } catch {}
            }, timeout);
        } else {
            await sock.groupSettingUpdate(chatId, 'not_announcement');
            await reply(sock, chatId, '✿ تم فتح المجموعة بنجاح.', msg);
        }
